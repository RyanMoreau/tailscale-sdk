...
export function OverviewCharts() {
	...
	const keyPostureData = useMemo(() => {
		...
		const reusable =
			keys?.filter((k) => Boolean(k.capabilities?.devices?.create?.reusable)).length ?? 0;
		const ephemeral =
			keys?.filter((k) => Boolean(k.capabilities?.devices?.create?.ephemeral)).length ?? 0;
		const preauthorized =
			keys?.filter((k) => Boolean(k.capabilities?.devices?.create?.preauthorized)).length ?? 0;
					...
					<div className="h-[260px] w-full min-w-0">
						<ResponsiveContainer width="100%" height="100%">
							...
							>
								...
								/>
								<Bar dataKey="value" radius={6} activeBar={{ fillOpacity: 0.9, stroke: "none" }} />
							</BarChart>
						</ResponsiveContainer>
					</div>
				</CardContent>
