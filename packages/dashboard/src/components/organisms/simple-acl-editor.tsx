...
export function SimpleAclEditor({ showHeader = true }: SimpleAclEditorProps) {
	...
	const removeGroup = (id: string) => {
		...
		setRules((prev) => prev.filter((rule) => rule.sourceGroup !== id && rule.targetGroup !== id));
	};
			...
			{showHeader ? (
				<div className="mb-8 space-y-3">
					...
					<p className="max-w-3xl text-base text-muted-foreground">
						Keep ACL changes fast and safe. Start with recipes, then use advanced rules only when
						needed.
					</p>
				</div>
			) : (
				...
				<TabsContent value="recipes" className="space-y-6">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>One-click policy recipes for incident response.</CardDescription>
						</CardHeader>
					...
					<div className="grid gap-4 lg:grid-cols-2">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Apply safe defaults first, then fine-tune in Rules.
								</CardDescription>
							</CardHeader>
					...
					<div className="grid gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
						<Card>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-1.5">
									...
									<Select
										value={newGroupIcon}
										onValueChange={(value) => setNewGroupIcon(value as IconKey)}
									>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Deleting a group also removes rules that reference it.
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-3">
								{groups.map((group) => (
					...
					<div className="grid gap-5 lg:grid-cols-[360px_minmax(0,1fr)]">
						<Card>
							<CardHeader>
								...
								<CardDescription>Choose source, target, and permission, then save.</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div className="space-y-1.5">
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Keep this list explicit for faster troubleshooting.
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-3">
								{rules.map((rule) => {
											...
											<div className="space-y-1.5">
												<div className="flex flex-wrap items-center gap-2 text-sm">
													<span className="text-muted-foreground">
														{renderIcon(getGroupIcon(rule.sourceGroup))}
													</span>
													<span className="font-medium">{getGroupName(rule.sourceGroup)}</span>
													<ArrowRight className="h-3 w-3 text-muted-foreground" />
													<span className="text-muted-foreground">
														{renderIcon(getGroupIcon(rule.targetGroup))}
													</span>
													<span className="font-medium">{getGroupName(rule.targetGroup)}</span>
													<Badge
														variant={rule.permission === "allow" ? "secondary" : "destructive"}
