...
export function OperatorOverview() {
  ...
  const pendingDevice = useMemo(
    () => devices?.find((device) => !device.authorized),
    [devices],
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={openNetwork ? "border-yellow-500/30 bg-yellow-500/10" : "border-green-500/30 bg-green-500/10"}>
        <CardContent className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button variant="outline" className="w-full sm:w-auto sm:justify-between" onClick={() => navigate("/acl")}>
      ...
      <Card>
        <CardHeader className="pb-4">
          ...
          <CardDescription>Do the 3 most common operations without leaving Overview.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-3">
          ...
          >
            <span className="space-y-1">
              ...
              <span className="block text-xs opacity-90">Generate a short-lived auth key preset</span>
            </span>
          ...
          >
            <span className="space-y-1">
              ...
              <span className="block text-xs text-muted-foreground">
                {pendingDevice ? pendingDevice.hostname || pendingDevice.name : "No pending devices"}
              </span>
            </span>
